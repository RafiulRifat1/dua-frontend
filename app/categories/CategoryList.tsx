...
interface Subcategory { id: number; category_id: number; name: string; slug: string; }
interface Dua { 
  id: number; category_id: number; subcategory_id: number; 
  title: string; arabic: string; reference: string; 
  transliteration:string; translation:string; slug: string; 
}
...
export default function CategoryList({ categories, subcategories, duas }: Props) {
  ...
  const activeCatRef = useRef<HTMLDivElement>(null); 
  ...
  const organizedData = useMemo(() => {
    ...
    const duasBySubcat = duas.reduce((acc, dua) => {
        if (!acc[dua.subcategory_id]) acc[dua.subcategory_id] = [];
        acc[dua.subcategory_id].push(dua);
        return acc;
    }, {} as Record<number, Dua[]>);

    const duasByCat = duas.reduce((acc, dua) => {
        acc[dua.category_id] = (acc[dua.category_id] || 0) + 1;
        return acc;
    }, {} as Record<number, number>);

    return { subcatMap, duasBySubcat, duasByCat };
  }, [subcategories, duas]); 
  ...
  useEffect(() => {
    ...
    if (!isNaN(idFromPath)) {
      setActiveSub(idFromPath);
      const sub = subcategories.find((s) => s.id === idFromPath);
      if (sub) {
        setActiveCatId(sub.category_id);
      }
    } else {
      setActiveSub(null);
    }
  ...
  useEffect(() => {
    if (activeCatId !== null && activeCatRef.current) {
  ...
  return (
    <div className="space-y-4">
      {categories.map((cat) => {
        ...
        const currentSubcategories = organizedData.subcatMap[cat.id] || [];
        const currentDuasCount = organizedData.duasByCat[cat.id] || 0;
          ...
          >
            ...
            <div
              ...
              className={`cursor-pointer rounded-xl p-4 flex items-center justify-between transition-all duration-300 ${
                isActive ? "bg-[#e8f0f5c2]" : " hover:bg-[#e8f0f57a]"
              }`}
            >
              ...
              <div className="relative ml-8 border-l-2 border-opacity-40 border-dashed border-[#417360]/40 pb-4">
                {currentSubcategories.length > 0 ? (
                  currentSubcategories.map((sub) => {
                    const subDuas = organizedData.duasBySubcat[sub.id] || [];
                    ...
                    return (
                      <div key={sub.id} className="relative pl-6 py-2 group/item cursor-pointer">
                        ...
                        <Link href={`/categories/${sub.slug}/${sub.id}`}>
                          <p className={`font-semibold opacity-70 text-[14px] leading-[22px] tracking-[0.01em] ${activeSub === sub.id ?"text-[#417360]" : "text-[#282E29]"} transition-colors`}>
                            {sub.name} 
                          </p>
                        </Link>
                        {activeSub === sub.id && (
                          <div className="">
                            {subDuas.map((dua: Dua) => (
                              <Link key={dua.id} href={`/categories/${sub.slug}/${sub.id}#dua-${dua.id}`}>
                                <div className="flex items-center gap-2 pl-4 py-1 font-regular text-sm text-[#282E29] hover:text-[#417360] transition-colors">
